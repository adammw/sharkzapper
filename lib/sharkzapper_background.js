...
function SharkZapperBackground(debug){
    ...
    console.log('isChrome',isChrome,'isFirefox',isFirefox,'isSafari',isSafari);
    ...
    if (isChrome) {
	     ...
	     */
        var SharkZapperPort = function(port) {
            this.name = "firefoxport";
	    }
        SharkZapperPort.prototype.send = function() {
            if(debug) console.log("sharkzapper:",">PORT:"+this.name+">",data.command, data);
	...
	sharkzapper.init = function init() {
        ...
        if (isChrome) chrome.browserAction.setTitle({"title":"sharkZapper"});
		return sharkzapper;
	}
